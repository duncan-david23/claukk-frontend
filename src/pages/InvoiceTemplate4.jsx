            ...
            <div ref={containerRef} className='container w-[793px] h-fit bg-white shadow-lg py-[20px] px-[50px]'>
                  ...
                  <h1 className='text-7xl' style={{ fontFamily: 'Great Vibes, cursive', color: customColor }} >Invoice</h1>
                    <div className='flex flex-col gap-[5px] w-[300px]' >
                      ...
                      <div className='flex justify-between text-sm'>
                        <h1 style={{color:customColor}}>INVOICE NO:</h1>
                        <h1 className='text-lg' style={{color:customColor}}>{invBinitials}{invoiceNumberValue}</h1>
                      </div>

                      <div className='flex justify-between text-sm'>
                        <h1>date:</h1>
                        <h1>{invoiceIssueDate}</h1>
                      </div>

                      <div className='flex justify-between text-sm'>
                         <div className='flex gap-[10px]  w-[100px] mt-[15px] items-center'>
                              <h1 className=' text-sm mt-[-20px]'>Due Date</h1>
                              <div>
                                {showEdtBtn &&
                                  <FiEdit3 onClick={()=>setShowEditDate(!showEditDate) } className='cursor-pointer mt-[-15px]'/>
                                }
                              </div>
                          </div>
                          <h1 className=''>{dueDateValue }</h1>
                      </div>
                      ...
                      <div className='flex justify-between items-center mt-[20px]'>
                        ...
                        <div className='flex gap-[10px]'>
                          <div className='flex gap-[10px]items-center'>
                              <h1 className='text-xl mt-[30px]'>BIll TO:</h1>

                                    {showEdtBtn && 
                                <FiEdit3 onClick={handleModal} className='mt-[20px] text-lg cursor-pointer'/>
                                }
                          ...
                          <div className='flex flex-col gap-[5px] mt-[30px] ml-[10px] text-xs'>
                            <div>
                              {displayCname && clientNameValue &&
                                <p className='w-[200px] h-[30px] p-[7px] ml-[-7px] '>{clientNameValue}</p>
                              }  
                            </div>
                            <div>
                              {displayCaddress && clientAddressValue &&
                                <p className='w-[200px] p-[7px] ml-[-7px] mt-[-9px]' >{clientAddressValue}</p>
                              }
                            </div>

                            <div>
                              {displayCemail && clientEmailValue &&
                                <p className='w-[200px]  p-[7px] ml-[-7px] mt-[-9px]' >{clientEmailValue}</p>
                              }
                            </div>

                            <div>
                              {displayCphone && clientPhoneValue &&
                                <p className='w-[200px] p-[7px] ml-[-7px] mt-[-9px]' >{clientPhoneValue}</p>
                              }
                            </div>
                          </div>
                        </div>

                        <p className='w-[420px] ml-[-80px] h-[1px] bg-black my-[20px]'></p>

                        <div className='flex gap-[10px]'>
                          <h1 className='text-xl' >FROM:</h1>
                          <div className='flex flex-col gap-[5px] mt-[5px]  text-xs'>
                            <h1>{businessData?.bsname || 'Company Name' }</h1>
                            <h1>{businessData?.bsnumber || 'Company Number' } / {businessData?.bsnumber2 || '-' }</h1>
                            <h1>{businessData?.bsmail || 'Company Email' }</h1>
                            <h1>{businessData?.bsaddress || 'Company Address' }</h1>
                          </div>
                        </div>
                  ...
                  <div className='relative mt-[50px] z-10'>
                     {showEdtBtn && <FaCirclePlus onClick={handleDisplayAddItemModal} className='absolute top-[25px] right-[-38px] text-xl cursor-pointer' />}
                        <table className='w-[720px]'>
                          <thead className='  border-b-2 border-t-2 border-black ' style={{borderColor: customColor}}>
                            <tr className='text-left'>
                              ...
                              <th className='px-[10px] py-[10px] text-center'>Qty</th>
                              <th className='px-[10px] py-[10px] text-center'>Price</th>
                              <th className='px-[20px] py-[10px] text-right'>Amount</th>
                            </tr>
                          ...
                          <tbody>
                            {itemData.map((item,index)=> {
                              ...
                              <tr key={index} className='relative mt-[10px] '>
                                ...
                                <td className='py-[20px] px-[10px] text-center'>{item.itemQty}</td>
                                <td className='py-[20px] px-[10px] text-center'>{currencySymbol}{itemPrice}</td>
                                <td className='py-[20px] px-[20px] text-right'>{currencySymbol}{total}</td>
                                {showEdtBtn && <CiCircleMinus onClick={()=> handleDeleteItem(index)} className='absolute top-[25px] right-[-10px] cursor-pointer'/>}
                              </tr>
                              </>
                            )})}
                        ...
                        <div className='mt-[30px]' >
                            <p className='w-[720px] h-[2px] bg-black' style={{backgroundColor:customColor}} ></p>
                              <div className='flex items-center justify-between my-[15px]' >
                                  <h1 className='font-semibold pl-[20px]' >SUBTOTAL</h1>
                                  <h1 className='font-semibold' >{currencySymbol}{theSubTotalAmount}</h1>
                              </div>
                            <p className='w-[720px] h-[2px] bg-black' style={{backgroundColor:customColor}} ></p>
                        </div>
                        ...
                        <div className='flex justify-end mt-[50px]'>
                            ...
                            <div className='w-[220px]'>
                              ...
                              <div className='mb-[20px] flex items-center justify-between text-sm mt-[20px]'>
                                <h1 className='font-bold' style={{color:customColor}} >TOTAL</h1>
                                <p className='font-bold' style={{color:customColor}}>{currencySymbol}{theTotalAmount}</p>
                              </div>
